...
/**
 * Главный класс JavaFX-приложения «Фейк магазин». <br><br>
 *
 * Отвечает за старт UI, показ главного меню, регистрацию/вход пользователя и выход из приложения.<br>
 * В методах и обработчиках событий добавлены перехваты исключений (try/catch) для безопасного UX.<br><br>
 *
...
public class Main extends Application {
    ...
    /**
     * Точка входа JavaFX-приложения.<br><br>
     *
     * Этот метод отвечает за инициализацию UI: подготовку меток, сцены и запуск главного меню.<br>
    ...
    /**
     * Отрисовывает главное меню (регистрация/вход/выход).<br>
     * Обновляет сцену, выставляет обработчики.<br><br>
     *
     * Комментарий по ошибкам: здесь важно ловить любые исключения при взаимодействии с tabPane и root,<br>
     * так как они могут быть модифицированы из других частей приложения. Вся критичная логика окружена<br>
    ...
    /**
     * <b>Экран входа:</b> проверяет наличие файла сохранения, показывает поля логина/пароля.<br>
     * При успешной проверке открывает магазин.<br><br>
     *
     *   <b>Подробности и уязвимые точки:</b><br>
     * - Чтение списка пользователей происходит из кэша {@link #userList}. При повреждении файла возможен<br>
     *   NPE или пустой список — это обрабатывается и отображается пользователю.<br>
     * - Обработчик нажатия кнопки входа содержит в себе логику поиска пользователя и проверки пароля;<br>
    ...
    /**
     * <b>Экран регистрации:</b> собирает имя/пароль/возраст/сумму, проверяет ввод, сохраняет пользователя.<br><br>
     *
     * <b>Критические моменты и пояснения:</b><br><br>
     * - Подсчёт нового ID пользователя выполняется через {@link AtomicInteger} — это простая эмпирическая<br>
     *   реализация для последовательного присвоения ID при старте приложения.<br>
     * - При парсинге строки суммы возможен {@link NumberFormatException} — это ловится и аккуратно<br>
     *   показывается пользователю как сообщение об ошибке.<br>
     * - Операция append(Path.of(PATH), getUser()) взаимодействует с файловой системой; возможны<br>
...
//            debugBordersFull(getTextCash());
    ...
    /**
     * Показывает подтверждение выхода и закрывает приложение по нажатию «Да».<br><br>
     *
     * Примечание: Platform.exit() вызывает завершение JavaFX-приложения. Любые исключения в этом<br>
    ...
    /**
     * Традиционный вход: делегирует запуск JavaFX.<br>
     * Добавлен перехват исключений на случай проблем запуска.<br>
